...
interface StructureConfig {
	...
	hidden?: number;
	out: number;
	activationFunc: IActivationFunction;
}
...
class Genome {
	...
	constructor(config: StructureConfig) {
		this.activationFunc = config.activationFunc;
		this.config = config;
		...
		while (stack.length) {
			...
			connections.forEach(connection => {
				...
				if (connection.getOutputNode().getState()) {
					...
					connection.getOutputNode().applyActivation(this.activationFunc);
	...
	randomConnectionStructure(): ConnectionStructure {
		let tries = 0;
	...
	addConnection(tNodes: ConnectionStructure, neat: NEAT): Connection {
